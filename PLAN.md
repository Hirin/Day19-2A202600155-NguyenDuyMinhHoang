...
**Prerequisite:** Docker Desktop ≥ 4.x, Python ≥ 3.10, port 6333/6380/5433 trống
...
```bash
netstat -ano | grep "6333"   # Qdrant HTTP
netstat -ano | grep "6380"   # Redis
netstat -ano | grep "5433"   # Postgres
```
...
POSTGRES_URL=postgresql://feast:feast@localhost:5433/feast_offline
```
...
1. **Start FastAPI server** (terminal riêng):
   ...
   .\.venv\Scripts\Activate.ps1
   make api
   # hoặc: uvicorn app.main:app --reload --port 8001
   ```

2. **Verify API hoạt động:**
   ```powershell
   curl "http://localhost:8001/search?q=cloud+computing&mode=hybrid"
   ```
...
| Rủi ro | Xử lý |
|---|---|
| Port conflict 6333/6380/5433 | `docker compose down` → kill process conflict → `docker compose up -d` |
